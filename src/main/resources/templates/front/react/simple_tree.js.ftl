...
const ${cfg.Model.entity}Page = (props)=> {
    ...
    // 从state中取出数据
    const { dataList, total, pagination, dataInfo, treeList , selectList , activeIndex } = ${cfg.Model.prefixName}Info;
    ...
    const onSelect = (keys, info) => {
        ...
        if(node && info.selected && node.children){
            const key = node.key;
            // 设置条件参数
            //pagination.parentId = key;
            fetchTable();
        }else if(node && info.selected && node.isLeaf){
            // 选中了子级树节点
            const key = node.key;
        }else{
            // 清空参数
